...
class AclLiteModel(object):
    ...
    def _gen_output_dataset(self, ouput_num):
        ...
        # print('in _gen_output_dataset:', num)
    ...
    def _release_dataset(self, dataset, free_memory=False):
        print('in acllite_model _release_dataset 1:')
        if not dataset:
            return
        print('in acllite_model _release_dataset 2')

        num = acl.mdl.get_dataset_num_buffers(dataset)
        print('in acllite_model _release_dataset 3')
        for i in range(num):
            data_buf = acl.mdl.get_dataset_buffer(dataset, i)
            if data_buf:
                self._release_databuffer(data_buf, free_memory)
        print('in acllite_model _release_dataset 4')

        ret = acl.mdl.destroy_dataset(dataset)
        if ret != const.ACL_SUCCESS:
            log_error("Destroy data buffer error ", ret)
        print('in acllite_model _release_dataset 5')
